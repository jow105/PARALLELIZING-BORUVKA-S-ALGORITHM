...
int find_parent(set* sets, int to_find)
{
    ...
    {
        omp_set_lock(&sets[sets[to_find].parent].mtx); // lock parent
        omp_unset_lock(&sets[to_find].mtx);//unlock previous
        sets[to_find].parent = find_parent(sets,sets[to_find].parent);

    }
    int k = sets[to_find].parent;
    omp_unset_lock(&sets[to_find].mtx); //unlock previous
...
int find_parent_h(set* sets, int to_find)
{
    omp_set_lock(&sets[to_find].mtx); //initial lock
    return find_parent(sets, to_find);
}

void union_sets(set* sets, int x, int y)
{
    
    int init_x = find_parent_h(sets,x);
    omp_set_lock(&sets[init_x].mtx);
    int init_y = find_parent_h(sets,y);
    omp_set_lock(&sets[init_y].mtx);
    ...
    omp_unset_lock(&sets[init_x].mtx);
    omp_unset_lock(&sets[init_y].mtx);
}
...
void run_boruvka(Graph* G)
{
    ...
    for(int i = 0; i < num_vertices; i++)
    {
        ...
        omp_init_lock(&ufind_sets[i].mtx);
    ...
    while(num_components > 1)
    {
        ...
        for(int j = 0; j < num_edges; j++)
        {
            ...
            int source = find_parent_h(ufind_sets,edges[j].source);
            int dest = find_parent_h(ufind_sets,edges[j].dest);
                ...
                if(cheap[k] != -1)
                {
                    ...
                    // cout << "actual source: " << edges[cheap[k]].source
                     // << ", actual dest: " << edges[cheap[k]].dest << " \n";
                        ...
                        int source = find_parent_h(ufind_sets,edges[cheap[k]].source);
                        int dest = find_parent_h(ufind_sets,edges[cheap[k]].dest);
...
// g++ -fopenmp -Wall -Wextra -O2 -std=c++11 boruvka_openmp_fine.cpp -lm
