 ...
 * bcchu - jwidjaja
 * BORUVKA'S ALGORITHM - lock_free_init
 */
...
bool updateRoot(set* sets, int x, int oldrank, int y, int newrank)
{
    set old = sets[x];
    ...
    set newset;
    newset.parent = sets[y].parent;
    newset.rank = newrank;
    return __sync_bool_compare_and_swap(&sets[x].parent, old.parent, newset.parent);
}
 
 
int find(set* sets, int to_find)
{
    set x =sets[to_find];
   
    while(x.parent != to_find)
    {
        set t = sets[x.parent];
        __sync_bool_compare_and_swap(&sets[x.parent].parent, t.parent, sets[t.parent].parent);
        x = sets[t.parent];
        to_find = t.parent;
    }
    return x.parent;
...
bool same(set* sets, int x, int y)  {
        while(1)
        {
            x = find(sets, x);
            y = find(sets, y);
...
void union_sets(set* sets, int x, int y)
{
    while(1)
    {
        ...
        if(xr > yr)
        {
            if(__sync_bool_compare_and_swap(&sets[x].parent, x, y))
            {
                return;
            }
        }
        ...
        {
            if(__sync_bool_compare_and_swap(&sets[y].parent, y, x))
            {
                return;
            }
        }
...
void run_boruvka(Graph* G)
{
    ...
    while(num_components > 1)
    {
        ...
        for(int j = 0; j < num_edges; j++)
        {
            ...
            int source = find(ufind_sets,edges[j].source);
            int dest = find(ufind_sets,edges[j].dest);
            if(source == dest)
            {  
...
// g++ -fopenmp -Wall -Wextra -O2 -std=c++11 boruvka_lock_free_init.cpp -lm
