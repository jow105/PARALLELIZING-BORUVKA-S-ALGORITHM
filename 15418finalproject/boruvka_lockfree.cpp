...
void run_boruvka(Graph* G)
{
    ...
    size_t temp = (size_t) num_vertices;
    // set* ufind_sets = (set*)malloc(sizeof(set)*num_vertices);
    
    UnionFind ufind_sets(temp);
    ...
    while(num_components > 1)
    {
        ...
        for(int j = 0; j < num_edges; j++)
        {
            ...
            int source = ufind_sets.find((size_t)edges[j].source);
            int dest = ufind_sets.find((size_t)edges[j].dest);
            if(source == dest)
            {  
        ...
        for(int k = 0; k < num_vertices; k++)
        {
                ...
                if(cheap[k] != -1)
                {
                    ...
                    size_t  source = ufind_sets.find((size_t)edges[k].source);
                    size_t dest = ufind_sets.find((size_t)edges[k].dest);
                    if(source == dest)
                    {
                        continue;
                    }
                        // uint32_t source = ufind_sets.find((uint32_t)edges[cheap[k]].source);
                        // uint32_t dest = ufind_sets.find((uint32_t)edges[cheap[k]].dest);
                    ...
                    ufind_sets.union_nodes(source,dest);
                    num_components--;
...
// g++ -fopenmp -Wall -Wextra -O2 -std=c++11 boruvka_lockfree.cpp -lm
